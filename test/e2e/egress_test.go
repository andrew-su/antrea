...
func TestEgress(t *testing.T) {
	...
	t.Run("testEgressMultiplePools", func(t *testing.T) { testEgressMultiplePools(t, data) })
...
func testEgressClientIP(t *testing.T, data *TestData) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			egress := data.createEgress(t, "egress-", matchExpressions, nil, "", egressNodeIP, nil, nil)
...
func testEgressClientIPFromVLANSubnet(t *testing.T, data *TestData) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			egress := data.createEgress(t, "egress-vlan", nil, map[string]string{"antrea-e2e": clientPod1}, pool.Name, "", nil, nil)
			defer data.crdClient.CrdV1beta1().Egresses().Delete(context.TODO(), egress.Name, metav1.DeleteOptions{})
...
func testEgressCRUD(t *testing.T, data *TestData) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			egress := data.createEgress(t, "crud-egress-", nil, map[string]string{"foo": "bar"}, pool.Name, "", nil, nil)
			defer data.crdClient.CrdV1beta1().Egresses().Delete(context.TODO(), egress.Name, metav1.DeleteOptions{})
...
func testEgressUpdateEgressIP(t *testing.T, data *TestData) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			egress := data.createEgress(t, "egress-", nil, map[string]string{"foo": "bar"}, originalPool.Name, "", nil, nil)
			defer data.crdClient.CrdV1beta1().Egresses().Delete(context.TODO(), egress.Name, metav1.DeleteOptions{})
...
func testEgressMultiplePools(t *testing.T, data *TestData) {
	selectNode0 := []metav1.LabelSelectorRequirement{
		{
			Key:      v1.LabelHostname,
			Operator: metav1.LabelSelectorOpIn,
			Values:   []string{nodeName(0)},
		},
	}
	ipRange0 := v1beta1.IPRange{CIDR: "10.10.10.0/30"}
	eip0 := data.createExternalIPPool(t, "eip-multi-az", ipRange0, nil, selectNode0, nil)
	defer data.crdClient.CrdV1beta1().ExternalIPPools().Delete(context.TODO(), eip0.Name, metav1.DeleteOptions{})

	selectNode1 := []metav1.LabelSelectorRequirement{
		{
			Key:      v1.LabelHostname,
			Operator: metav1.LabelSelectorOpIn,
			Values:   []string{nodeName(1)},
		},
	}
	ipRange1 := v1beta1.IPRange{CIDR: "20.20.20.0/30"}
	eip1 := data.createExternalIPPool(t, "eip-multi-az-secondary-", ipRange1, nil, selectNode1, nil)
	defer data.crdClient.CrdV1beta1().ExternalIPPools().Delete(context.TODO(), eip1.Name, metav1.DeleteOptions{})

	externalIPPools := []string{eip0.Name, eip1.Name}

	egress := data.createEgress(t, "multi-az-egress-", nil, map[string]string{"foo": "bar"}, "", "", externalIPPools, nil)
	defer data.crdClient.CrdV1beta1().Egresses().Delete(context.TODO(), egress.Name, metav1.DeleteOptions{})

	// check the status.EgressIP instead of spec.EgressIP
	egress, err := data.checkEgressState(egress.Name, "10.10.10.1", nodeName(0), "", time.Second)
	require.NoError(t, err)

	eip0ToUpdate, err := data.crdClient.CrdV1beta1().ExternalIPPools().Get(context.TODO(), eip0.Name, metav1.GetOptions{})
	require.NoError(t, err, "Failed to get ExternalIPPool %v", eip0ToUpdate)
	eip0ToUpdate.Spec.NodeSelector.MatchExpressions = []metav1.LabelSelectorRequirement{
		{
			Key:      v1.LabelHostname,
			Operator: metav1.LabelSelectorOpIn,
			Values:   []string{"fake-node"},
		},
	}
	_, err = data.crdClient.CrdV1beta1().ExternalIPPools().Update(context.TODO(), eip0ToUpdate, metav1.UpdateOptions{})
	require.NoError(t, err, "Failed to create ExternalIPPool")

	// check the status.EgressIP instead of spec.EgressIP
	egress, err = data.checkEgressState(egress.Name, "20.20.20.1", nodeName(1), "", time.Second)
	require.NoError(t, err)
}
...
func testEgressMigration(t *testing.T, data *TestData, triggerFunc, revertFunc func(poolName, evictNode string), checkEvictNode bool, timeout time.Duration, ipRange *v1beta1.IPRange) {
	...
	egress := data.createEgress(t, "migration-egress-", nil, map[string]string{"foo": "bar"}, externalIPPoolTwoNodes.Name, "", nil, nil)
	defer data.crdClient.CrdV1beta1().Egresses().Delete(context.TODO(), egress.Name, metav1.DeleteOptions{})
...
func testEgressUpdateBandwidth(t *testing.T, data *TestData) {
	...
	egress := data.createEgress(t, "egress-qos-", nil, map[string]string{"antrea-e2e": clientPodName}, "", egressNodeIP, nil, bandwidth)
...
func (data *TestData) checkEgressState(egressName, expectedIP, expectedNode, otherNode string, timeout time.Duration) (*v1beta1.Egress, error) {
	...
	pollErr := wait.PollUntilContextTimeout(context.Background(), 200*time.Millisecond, timeout, true, func(ctx context.Context) (bool, error) {
		...
		if egress.Status.EgressIP != expectedIP {
			return false, nil
		}
...
func (data *TestData) createEgress(t *testing.T, generateName string,
	matchExpressions []metav1.LabelSelectorRequirement, matchLabels map[string]string,
	externalPoolName string, egressIP string, externalIPPools []string, bandwidth *v1beta1.Bandwidth) *v1beta1.Egress {
