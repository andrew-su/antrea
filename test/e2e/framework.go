...
// getFlowAggregator retrieves the name of the Flow-Aggregator Pod (flow-aggregator-*) with a specific label.
func (data *TestData) getFlowAggregator(aggregatorIndex int) (*corev1.Pod, error) {
	...
	if len(pods.Items) > 1 {
		return nil, fmt.Errorf("expected *exactly* one Pod with label 'app=%s' but got %d Pods", match, len(pods.Items))
	}
