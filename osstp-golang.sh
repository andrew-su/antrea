...
echo ===================================================
wget https://osm.eng.vmware.com/utilities/osstpclients3.zip
unzip osstpclients3.zip -d /tmp/
os=`uname`
case $os in
  "Linux")
	  cp /tmp/bin/client_executables/linux-amd64/osstp-load .
	  ;;
  "Darwin")
	  cp /tmp/bin/client_executables/darwin/osstp-load .
	  ;;
  *)
	  echo $os, "unsupported"
          exit 1
