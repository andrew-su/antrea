...
class Target(object):
    ...
    def GetCommands(self, hosttype):
        ...
        Each dictionary should contain:
           ...
           'desc'    - Returns a short, human readible description of what
                       the task is doing.
           'root'    - The directory where the command should be run.
    ...
    def ExtractVersionFromProvenanceMetadata(self, provenance_file=None):
        """
        Open the provenance file and pull out the first version. The file is
        ...
        - foo/src:
           - name: foo
             version: "1.2.3"
             licenses: [BSD]
             uses: [dynamic_with_oss]
             url: http://www.example.org/foo.1.2.3.tgz
             modified: true
        """
        import yaml
        if not provenance_file:
            provenance_file = self._target_real_name + '.yaml'
        path = os.path.join(
            os.path.dirname(__file__), '..', 'provenance', provenance_file)
        with open(path) as f:
            data = yaml.load(f.read())
        return data[0].values()[0][0]['version']
