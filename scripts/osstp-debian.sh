...
EOF
./osstp-load.py --noinput -A /tmp/osm-apykey -R Antrea/1.0.0-0.9.0 --baseos-srcdir ~/source osstpmgt.yaml | tee oss.log
...
if [ -f osstpmgt.yaml ]; then
  ./osstp-load.py --noinput --debug -A /tmp/osm-apykey -R Antrea/1.0.0-0.9.0 --baseos-srcdir ~/source osstpmgt.yaml
fi
