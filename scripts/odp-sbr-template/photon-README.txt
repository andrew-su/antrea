This document demonstrates how to install and build a package on the full version of Photon OS from the package's source RPM.
...
It is recommended that you use the full version of Photon OS to install and build packages from their source RPMs. Do not use the  minimal version to work with source RPMs.  
...
To install the source file, run the following command with your unprivileged user account, replacing the sed example source RPM with the name of the one that you want to install: 
	...
	rpm -i /tmp/sed-4.4-3.ph2.src.rpm
...
If successful, the rpmbuild -ba command builds the RPM and generates an RPM package file in your ~/rpmbuild/RPMS/x86_64 directory. Example:
	...
	ls RPMS/x86_64/
	sed-4.4-3.x86_64.rpm  sed-debuginfo-4.4-3.x86_64.rpm  sed-lang-4.4-3.x86_64.rpm
...
To install the RPM, run the following command with your unprivileged user account:  
	...
	rpm -i RPMS/x86_64/sed-4.4-3.x86_64.rpm
