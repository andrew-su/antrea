...
ARTIFACTORY_URL="antrea-docker-dev-local.usw5.packages.broadcom.com"
...
if [[ $# -ne 3 ]]; then
    ...
    echo "  artifactory_user: usw5.packages.broadcom.com user name (Broadcom ID without @broadcom.net)"
    echo "  artifactory_token: user's token to login usw5.packages.broadcom.com"
...
echo ====== Promoting All Images to Broadcom Artifactory usw5.packages.broadcom.com ======
...
for image in $images; do
  ...
  curl -i -u$artifactory_user:$artifactory_token -X POST "https://usw5.packages.broadcom.com/artifactory/api/docker/antrea-docker-dev-local/v2/promote" -H "Content-Type: application/json" -d "{\"targetRepo\":\"antrea-docker-prod-local\",\"dockerRepository\":\"$image\"}"
done
