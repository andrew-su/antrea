...
EOF
./osstp-load.py --noinput -A /tmp/osm-apykey -R Antrea/1.3.0-1.2.2 --baseos-srcdir ~/source osstpmgt.yaml | tee oss.log
...
if [ -f bin/osstpmgt.yaml ]; then
  ./bin/osstp-load.py --noinput --debug -A /tmp/osm-apykey -R Antrea/1.3.0-1.2.2 --baseos-srcdir ../source bin/osstpmgt.yaml || true
fi
