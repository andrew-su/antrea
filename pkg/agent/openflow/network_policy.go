...
type clause struct {
	...
	// dryRun is a boolean dictating whether this flows generated by this clause should only be used for dry-run
	dryRun bool
}
...
func (c *policyRuleConjunction) calculateClauses(rule *types.PolicyRule) (uint8, binding.Table, binding.Table) {
	...
	clauseFn := c.newClause
	if rule.DryRun {
		clauseFn = c.newDryRunClause
	}
	...
	if rule.From != nil {
		...
		c.fromClause = clauseFn(fromID, nClause, ruleTable, defaultTable)
	}
	if rule.To != nil {
		...
		c.toClause = clauseFn(toID, nClause, ruleTable, defaultTable)
	}
	if rule.Service != nil {
...
func (f *featureNetworkPolicy) initFlows() []*openflow15.FlowMod {
	...
	flows = append(flows, f.dryRunDefaultFlows()...)
...
func (f *featureNetworkPolicy) dryRunDefaultFlows() []binding.Flow {
	var flows []binding.Flow
	cookieID := f.cookieAllocator.Request(f.category).Raw()

	// If there are no matching metrics
	flows = append(flows,
		EgressMetricTable.ofTable.BuildFlow(priorityLow-10).
			Cookie(cookieID).
			MatchRegMark(DryRunHitRegMark).
			Action().LoadToRegField(APDenyRegMark.GetField(), 0x0).
			Action().LoadRegMark(DryRunLoggedRegMark).
			Action().LoadRegMark(DryRunPassHitRegMark).
			Action().ResubmitToTables(AntreaPolicyEgressRuleTable.GetID()).
			Done(),
		IngressMetricTable.ofTable.BuildFlow(priorityLow-10).
			Cookie(cookieID).
			MatchRegMark(DryRunHitRegMark).
			Action().LoadToRegField(APDenyRegMark.GetField(), 0x0).
			Action().LoadRegMark(DryRunLoggedRegMark).
			Action().LoadRegMark(DryRunPassHitRegMark).
			Action().ResubmitToTables(AntreaPolicyIngressRuleTable.GetID()).
			Done(),
	)
	return flows
}
